...
/**
 * Opened from a checklist row's pencil button: edit the item's content, or delete it
 * (behind a one-time confirmation) via the same dialog's "삭제" action.
 */
@Composable
fun ChecklistEditDialog(
    ...
    Dialog(onDismissRequest = onDismiss) {
        Surface(shape = RoundedCornerShape(20.dp), color = YwSurface) {
            Column(
                ...
                Text(
                    text = stringResource(R.string.checklist_edit_title),
                    color = YwTextPrimary,
                    fontSize = 16.sp,
    ...
    if (showDeleteConfirm) {
        AlertDialog(
            ...
            text = { Text(stringResource(R.string.checklist_delete_confirm_message)) },
            confirmButton = {
